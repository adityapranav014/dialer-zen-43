...
// ─── Avatar helpers (mirrors AdminDashboard) ─────────────────────────────────
...
const ActivityDescription = ({ text, statusPillMap, statusLabels }: { text: string; statusPillMap: Record<string, string>; statusLabels: string[] }) => {
    // Match "→ Status" at end of description — dynamic labels
...
const BdaDashboard = () => {
    ...
    const myStatsDisplay = [
        { label: "Calls Today", value: stats?.totalCalls ?? 0, icon: PhoneCall, colour: "text-blue-500", bg: "bg-blue-50 dark:bg-blue-950/30" },
        { label: "Talk Time", value: `${stats?.totalMinutes ?? 0}m`, icon: Timer, colour: "text-violet-500", bg: "bg-violet-50 dark:bg-violet-950/30" },
        { label: "Conversions", value: stats?.conversions ?? 0, icon: TrendingUp, colour: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-950/30" },
        { label: "Rank", value: myRank ? `#${myRank}` : "—", icon: Star, colour: "text-amber-500", bg: "bg-amber-50 dark:bg-amber-950/30" },
    ];
        ...
        <div className="flex flex-col md:flex-1 md:min-h-0">
                ...
                {myStatsDisplay.map((stat) => (
                    <div key={stat.label} className="surface-float px-4 py-3 flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                            <p className="text-lg font-bold tracking-tight leading-none text-foreground">{stat.value}</p>
                            <p className="text-[11px] text-foreground/45 font-medium mt-0.5 truncate">{stat.label}</p>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="flex flex-col gap-4 md:flex-1 md:min-h-0 md:overflow-hidden">
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 md:flex-1 md:min-h-0">
                    {/* Top Performers — table style matching admin */}
                        ...
                        <div className="md:flex-1 md:min-h-0 md:overflow-y-auto scroll-container px-5 pb-4">
                            {leaderboard.length === 0 ? (
                                <div className="py-10 text-center">
                                    <p className="text-sm text-foreground/40">No team data yet</p>
                                    <p className="text-xs text-foreground/25 mt-1">Leaderboard will populate as the team logs calls</p>
                                </div>
                            ) : (
                                <table className="w-full text-xs">
                                    <thead className="sticky top-0 bg-card z-10">
                                        <tr className="border-b border-border">
                                            ...
                                            <th className="text-right pb-3 px-2 text-[11px] font-semibold text-foreground/30 uppercase tracking-widest">Hit %</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        {leaderboard.map((entry, idx) => {
                                            ...
                                            return (
                                                <tr
                                                    key={entry.id}
                                                    className={`border-b border-foreground/[0.04] last:border-0 transition-all duration-200 ${
                                                        isMe ? "bg-accent" : "hover:bg-accent/50"
                                                    }`}
                                                >
                                                    ...
                                                    <td className="py-3 px-2 text-right">
                                                        <span className="font-semibold text-foreground text-[13px]">{entry.calls}</span>
                                                    </td>
                                                    <td className="py-3 px-2 text-right hidden sm:table-cell">
                                                        <span className="text-foreground/55 font-medium">{entry.talkTimeMins}m</span>
                                                    </td>
                                                    <td className="py-3 px-2 text-right">
                                                        <span className="font-semibold text-emerald-600 text-[13px]">{entry.conversions}</span>
                                                    </td>
                                                    <td className="py-3 px-2 text-right">
                                                        <HitRateBadge calls={entry.calls} conversions={entry.conversions} />
                                                    </td>
                                                </tr>
                                            );
                                        })}
                    ...
                    {/* Right column — Activity + Talk Time */}
                    <div className="flex flex-col gap-4 md:min-h-0">
                        {/* My Activity */}
                        <BentoCard className="md:flex md:flex-col md:min-h-0 md:flex-1 !p-0 overflow-hidden">
                            <div className="shrink-0 flex items-center justify-between px-6 pt-5 pb-3">
                                <h3 className="text-sm font-bold text-foreground flex items-center gap-2">
                            ...
                            <div className="md:flex-1 md:min-h-0 md:overflow-y-auto scroll-container px-6 pb-4">
                                {myActivities.length === 0 ? (
                                    <div className="py-10 text-center">
                                        ...
                                        <p className="text-xs text-foreground/25 mt-1">Make your first call to get started</p>
                                    </div>
                                ) : (
                                    <div className="space-y-4">
                                        {myActivities.map((activity) => (
                                            <div key={activity.id} className="flex items-start gap-3">
                                                <div className={`h-1.5 w-1.5 mt-[7px] rounded-full shrink-0 ${activityDotClass[activity.action] ?? "bg-foreground/20"}`} />
                                                <div className="min-w-0">
                                                    <p className="text-[13px] text-foreground leading-snug"><ActivityDescription text={activity.description} statusPillMap={statusPillMap} statusLabels={statusLabels} /></p>
