...
import { ChevronRight, Settings, Moon, Sun, Monitor, LogOut, Maximize, Minimize } from "lucide-react";
...
export const AppLayout = ({
  ...
  return (
    <div className="h-[100dvh] w-screen overflow-hidden flex bg-background">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-[100dvh] overflow-hidden">
        ...
        <header className="shrink-0 h-14 flex items-center justify-between px-4 sm:px-6 border-b border-border bg-card z-30 shadow-[0_1px_2px_rgba(0,0,0,0.03)]">
          ...
          <div className="flex items-center gap-1.5 text-sm min-w-0">
            <button
              ...
              className="text-foreground/50 font-medium hidden sm:inline hover:text-foreground cursor-pointer transition-all duration-200"
            >
              DialFlow
            </button>
            {currentTenantName && (
              <>
                <ChevronRight className="h-3.5 w-3.5 text-foreground/35 hidden sm:inline shrink-0" />
                <button
                  onClick={() => navigate("/dashboard")}
                  className="text-foreground/50 font-medium hidden sm:inline truncate max-w-[120px] hover:text-foreground cursor-pointer transition-all duration-200"
                >
                  {currentTenantName}
                </button>
              </>
              ...
              <PopoverContent align="end" sideOffset={8} className="w-72 p-0 rounded-2xl border border-border shadow-xl bg-card">
                ...
                <div className="p-4 flex items-center gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <span className="inline-block mt-1 text-[10px] font-semibold px-2 py-0.5 rounded-md bg-accent text-foreground/80">
                      {roleLabel}
                    </span>
                  </div>
                </div>
                ...
                <div className="py-1.5">
                  <p className="px-4 pt-2 pb-1 text-[10px] font-semibold uppercase tracking-widest text-foreground/45">Account</p>
                  {accountItems.map((item) => (
                    ...
                    >
                      ...
                      <div className="min-w-0 text-left">
                        ...
                        <p className="text-[10px] text-foreground/45">{item.desc}</p>
                      </div>
                    </button>
                  ))}
                  ...
                  <div className="px-4 py-2">
                    ...
                    >
                      ...
                      <div className="min-w-0 text-left flex-1">
                        ...
                        <p className="text-[10px] text-foreground/45">Theme & display</p>
                      </div>
                ...
                <div className="px-4 pb-3 pt-1">
                  <p className="text-[10px] text-foreground/40 text-center font-medium">DialFlow v1.0.0</p>
                </div>
              </PopoverContent>
            </Popover>
          </div>
        ...
        <main className="flex-1 min-h-0 overflow-hidden">
          <div className={`h-full ${fullHeight ? "overflow-y-auto md:overflow-hidden md:flex md:flex-col scroll-container md:scroll-container-none" : "overflow-y-auto scroll-container"}`}>
            <div className={`${maxWidthClass} mx-auto w-full px-4 sm:px-6 pt-6 ${fullHeight ? "pb-24 md:pb-6 md:flex-1 md:flex md:flex-col md:min-h-0" : "pb-24 md:pb-6"}`}>
              {children}
            </div>
          </div>
        </main>
      ...
      {/* PWA install prompt — shown once after login */}
      <PWAInstallBanner />
    </div>
  );
};
